...
class App:
    def __init__(self, master: ctk.CTk):
        ...
        self.rows_label = ctk.CTkLabel(self.master, text=f"number of rows {self.row_number}", font = label_font)
        ...
        self.balls_drawn_label = ctk.CTkLabel(self.master, text=f"balls to draw {self.balls_drawn}", font = label_font)
        ...
        self.max_ball_number_label = ctk.CTkLabel(self.master, text=f"max ball number {self.max_ball_number}", font = label_font)
        self.max_ball_number_label.place(relx=slider_pos, rely=0.5, anchor="e")
        #Create the max ball number slider
        slider_max_ball_number = ctk.CTkSlider(self.master, from_=12, to=99, number_of_steps=87, command=self.slider_max_ball_number_event)
        ...
        self.powerball_label = ctk.CTkLabel(self.master, text=f"powerball max {self.powerball}", font = label_font)
        self.powerball_label.place(relx=slider_pos, rely=0.7, anchor="e")
        #Create the powerball slider
        slider_powerball = ctk.CTkSlider(self.master, from_=0, to=99, number_of_steps=100, command=self.slider_powerball_event)
        slider_powerball.place(relx=slider_pos, rely=0.8, anchor="e")
        slider_powerball.set(int(self.powerball))
        
        
        #Create the button
        self.button = ctk.CTkButton(self.master, text="Generate Numbers", command=self.populate_box, corner_radius=10, font = label_font)
        self.button.place(relx=slider_pos, rely=0.9, anchor="e")

        #create the text box
        self.text_box = ctk.CTkTextbox(self.master, border_spacing=25, width=450, height=500, font=("Courier bold", 23), text_color = 'Black', corner_radius = 10, fg_color = 'grey')
